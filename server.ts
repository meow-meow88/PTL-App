...
`;
  ...
  app.post('/api/gemini/molly-payment-evidence', async (req: Request, res: Response) => {
    ...
    try {
      const context = JSON.stringify({message: String(message || '').slice(0, 2500),
        invoices: invoices.slice(0, 30).map((i: any) => ({id: i.id, number: i.invoiceNumber, jobId: i.jobId,
      ...
      const response = await ai.models.generateContent({model: 'gemini-3.8-flash',
        ...
        config: {responseMimeType: 'application/json', responseSchema: {type: Type.OBJECT, properties: {
          transfers: {type: Type.ARRAY, items: {type: Type.OBJECT, properties: {
            fileIndex: {type: Type.INTEGER}, amount: {type: Type.NUMBER}, reference: {type: Type.STRING},
            date: {type: Type.STRING}, purpose: {type: Type.STRING}, invoiceId: {type: Type.STRING},
            jobId: {type: Type.STRING}, reason: {type: Type.STRING}, confidence: {type: Type.STRING},
          }, required: ['fileIndex', 'amount', 'reference', 'date', 'purpose', 'invoiceId', 'jobId', 'reason', 'confidence']}},
        }, required: ['transfers']}}});
      const parsed = JSON.parse(response.text || '{}');
      const transfers = Array.isArray(parsed.transfers) ? parsed.transfers.filter((t: any) =>
        ...
        Number.isFinite(t.amount) && t.amount > 0).map((t: any) => ({...t,
          ...
          jobId: jobs.some((j: any) => j.id === t.jobId) ? t.jobId : '',
        })) : [];
      res.json({transfers});
    } catch (error) {
      console.error('Molly payment evidence:', error);
      res.status(502).json({error: 'Could not read the documents. Check the files and try again.'});
    }
  });
